...
import {
  ...
  deleteResumeObjects, resumeKey, photoKey, photoThumbKey,
...
describe('storage-private key builders', () => {
  it('resumeKey', () => {
    assert.equal(resumeKey('U1', 'R1'), 'users/U1/resumes/R1.json');
  });
  it('photoKey', () => {
    assert.equal(photoKey('U1', 'R1', 'jpg'), 'users/U1/photos/R1.jpg');
  });
  it('photoThumbKey', () => {
    assert.equal(photoThumbKey('U1', 'R1'), 'users/U1/photos/R1-thumb.jpg');
  });
});
...
describe('storage-private I/O', () => {
  ...
  it('deleteResumeObjects removes the JSON + all photo keys we know about', async () => {
    ...
    assert.ok(keys.includes('users/U1/resumes/R1.json'));
    assert.ok(keys.includes('users/U1/photos/R1.jpg'));
    assert.ok(keys.includes('users/U1/photos/R1.png'));
    assert.ok(keys.includes('users/U1/photos/R1.webp'));
    assert.ok(keys.includes('users/U1/photos/R1-thumb.jpg'));
  });
});
```
...
export const photoKey = (customId, resumeId, ext) => `users/${customId}/photos/${resumeId}.${ext}`;
export const photoThumbKey = (customId, resumeId) => `users/${customId}/photos/${resumeId}-thumb.jpg`;
...
export const deleteResumeObjects = async ({ bucket, customId, resumeId, client = s3 }) => {
  const keys = [
    ...
    ...PHOTO_EXTS.map((ext) => photoKey(customId, resumeId, ext)),
    photoThumbKey(customId, resumeId),
  ];
...
export const buildPhotoPutKey = ({ customId, resumeId, contentType }) => {
  const ext = contentType === 'image/png' ? 'png'
            : contentType === 'image/webp' ? 'webp'
            : 'jpg';
  return photoKey(customId, resumeId, ext);
};
```
...
describe('GET /api/resumes/{id}', () => {
  it('returns { resume, etag } on hit + ETag header', async () => {
    s3.on(GetObjectCommand).resolves({
      Body: { transformToString: async () => JSON.stringify({ id: 'R1', ownerCustomId: 'U1', title: 'A', templateId: 'monaco', paperSize: 'A4', sections: [] }) },
      ETag: '"abc"',
    });
    ...
    assert.deepEqual(JSON.parse(res.body).resume.id, 'R1');
    assert.deepEqual(JSON.parse(res.body).etag, '"abc"');
  });
  ...
  it('returns 403 when ownerCustomId does not match caller', async () => {
    // Defense-in-depth: IAM already prevents cross-user reads, but S3 lifecycle bugs could leak.
    s3.on(GetObjectCommand).resolves({
...
import { extractUser } from '../lib/auth.js';
import { getResume } from '../lib/storage-private.js';
import { config } from '../config.js';
...
export const getResumeRoute = async (event) => {
  ...
  return withEtag(ok({ resume: got.resume, etag: got.etag }), got.etag);
};
```
...
const evt = (over = {}) => ({
  ...
  body: JSON.stringify({ contentType: 'image/jpeg', ...over }),
});

describe('POST /api/resumes/{id}/photo', () => {
  it('returns uploadUrl + key for a valid JPEG request on an owned resume', async () => {
    // Resume existence + ownership check:
    s3.on(GetObjectCommand).resolves({
      Body: { transformToString: async () => JSON.stringify({ id: 'R1', ownerCustomId: 'U1', title: 'A', templateId: 'monaco', paperSize: 'A4', sections: [] }) },
      ETag: '"e"',
    });
    ...
    assert.equal(res.statusCode, 200);
    const body = JSON.parse(res.body);
    assert.equal(body.key, 'users/U1/photos/R1.jpg');
    assert.match(body.uploadUrl, /^https:\/\//);
  ...
  it('uses .png for image/png', async () => {
    ...
    const res = await photoUploadUrl(evt({ contentType: 'image/png' }));
    assert.equal(JSON.parse(res.body).key, 'users/U1/photos/R1.png');
  });
  ...
  it('rejects unsupported content type with 400', async () => {
    ...
    const res = await photoUploadUrl(evt({ contentType: 'image/gif' }));
    assert.equal(res.statusCode, 400);
  });
...
import { ok, error, parseBody } from '../lib/http.js';
import { extractUser } from '../lib/auth.js';
import { getResume, buildPhotoPutKey } from '../lib/storage-private.js';
import { config } from '../config.js';
...
export const photoUploadUrl = async (event) => {
  ...
  const body = parseBody(event);
  if (!SUPPORTED.has(body.contentType)) {
    return error(400, 'BadRequest', `contentType must be one of ${[...SUPPORTED].join(', ')}`);
  }
  ...
  const key = buildPhotoPutKey({ customId: user.customId, resumeId, contentType: body.contentType });
  ...
  const uploadUrl = await getSignedUrl(
    ...
    new PutObjectCommand({
      ...
      ContentLength: body.contentLength,  // optional client-provided hint
    }),
    { expiresIn: EXPIRES },
  );

  return ok({ uploadUrl, key, expiresIn: EXPIRES, maxBytes: MAX_BYTES });
};
```
